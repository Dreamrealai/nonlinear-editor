...
export const GET = withErrorHandling(async (request: NextRequest) => {
  ...
  if (statusData.status === 'COMPLETED') {
    ...
    if (!audioUrl) {
      return NextResponse.json({ error: 'No audio URL in result' }, { status: 500 });
    }
    ...
    try {
      ...
      clearTimeout(timeout3);
      if (error instanceof Error && error.name === 'AbortError') {
        return NextResponse.json({ error: 'Audio download timeout after 60s' }, { status: 504 });
      }
      throw error;
    }
    ...
    if (uploadError) {
      serverLogger.error(
        { error: uploadError, userId: user.id, projectId, requestId },
    ...
    if (assetError) {
      ...
      return NextResponse.json({ error: 'Failed to save audio asset metadata' }, { status: 500 });
    }
