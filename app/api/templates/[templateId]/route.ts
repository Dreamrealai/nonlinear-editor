 ...
 */
export const PATCH = withAuth<{ templateId: string }>(
  async (req, { user, supabase }, routeContext) => {
    try {
      const params = await routeContext?.params;
      const templateId = params?.templateId;
      ...
      if (
        body.timeline_data !== undefined &&
        (typeof body.timeline_data !== 'object' || body.timeline_data === null)
      ) {
        throw new ValidationError(
          'timeline_data must be an object',
          'timeline_data',
          'INVALID_TYPE'
        );
      }
...
export const DELETE = withAuth<{ templateId: string }>(
  async (_, { user, supabase }, routeContext) => {
    try {
      ...
      if (deleteError) {
        serverLogger.error(
          { error: deleteError, userId: user.id, templateId },
