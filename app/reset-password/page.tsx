...
export default function ResetPasswordPage(): React.JSX.Element {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to home after 2 seconds - middleware will handle session
      setTimeout(() => {
        router.push('/');
      }, 2000);
    } catch (err) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-neutral-50">
      <div className="w-full max-w-md space-y-8 rounded-xl border border-neutral-200 bg-white p-8 shadow-sm">
        <div>
          <h2 className="text-center text-3xl font-bold text-neutral-900">Reset Password</h2>
          <p className="mt-2 text-center text-sm text-neutral-600">Enter your new password below</p>
        </div>

        {success ? (
          <div className="space-y-6">
            <div className="rounded-lg bg-green-50 px-4 py-3 text-sm text-green-600">
              Password reset successful! You&apos;re now logged in. Redirecting...
            </div>
          </div>
        ) : (
