/**
 * Extended Tests for POST /api/export
...
describe('POST /api/export - Extended Coverage', () => {
  ...
  it('should return 401 for unauthenticated user', async () => {
    ...
    const request = new NextRequest('http://localhost/api/export', {
      method: 'POST',
      body: JSON.stringify({ projectId: 'proj-123', format: 'mp4' }),
    });
  ...
  it('should handle invalid format', async () => {
    ...
    const request = new NextRequest('http://localhost/api/export', {
      method: 'POST',
      body: JSON.stringify({ projectId: 'proj-123', format: 'invalid-format' }),
    });
