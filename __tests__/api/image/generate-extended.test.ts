/**
 * Extended Tests for POST /api/image/generate
...
jest.mock('@/lib/imagen', (): Record<string, unknown> => ({ generateImage: jest.fn() }));
jest.mock(
  '@/lib/rateLimit',
  (): Record<string, unknown> => ({
...
describe('POST /api/image/generate - Extended Coverage', () => {
  beforeEach(() => resetAllMocks());

  it('should return 401 for unauthenticated user', async () => {
    mockUnauthenticatedUser();
    const request = new NextRequest('http://localhost/api/image/generate', {
      method: 'POST',
      body: JSON.stringify({ prompt: 'test image', projectId: 'proj-123' }),
    });
