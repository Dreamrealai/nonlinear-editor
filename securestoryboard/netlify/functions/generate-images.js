...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    return errorResponse(500, 'Server configuration error', 'Image generation service not configured. Please contact support.', 'configuration_error');
  }
  ...
  try {
    ...
    if (!prompt) {
      return errorResponse(400, 'Missing prompt', 'A prompt is required to generate images', 'validation_error');
    }

    const apiUrl = FAL_URLS[model];
    if (!apiUrl) {
      return errorResponse(400, 'Invalid model', `Model "${model}" is not supported. Available models: imagen3, imagen4`, 'validation_error');
    }
    ...
    try {
      ...
      return errorResponse(502, 'External API error', 'Failed to connect to image generation service. Please try again.', 'network_error');
    }
    ...
    try {
      ...
      return errorResponse(502, 'Invalid API response', 'Received invalid response from image generation service', 'api_error');
    }
    ...
    if (!result.images || !Array.isArray(result.images)) {
      ...
      return errorResponse(502, 'Invalid response format', 'Image generation service returned unexpected format', 'api_error');
    }
    ...
    if (error.name === 'AbortError' || error.message.includes('timeout')) {
      return errorResponse(504, 'Request timeout', 'Image generation took too long. Please try again.', 'timeout_error');
    }
