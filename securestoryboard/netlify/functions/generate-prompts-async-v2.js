...
exports.handler = async (event, context) => {
  ...
  if (event.httpMethod === 'GET') {
    ...
    try {
      ...
      if (jobData.status === 'completed' || jobData.status === 'failed') {
        ...
        setTimeout(() => store.delete(jobId).catch(() => {}), 60000); // Delete after 1 minute
      }
  ...
  try {
    ...
    // Trigger background function via internal endpoint
    // Note: This is a workaround since Netlify doesn't have true background functions
    // We'll use a webhook or scheduled function approach
    
    // For now, we'll process inline but with early response
    // The key is to return immediately and let the processing happen asynchronously
    process.nextTick(async () => {
      try {
        await processJobAsync(jobId, requestData, store);
      } catch (error) {
...
async function processJobAsync(jobId, requestData, store) {
