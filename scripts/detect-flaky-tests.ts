...
/**
 * Flaky Test Detector (TypeScript)
 *
 ...
 *
 * Usage:
 *   npm run test:flaky [iterations] [pattern]
 *   tsx scripts/detect-flaky-tests.ts [iterations] [pattern]
 *
 * Arguments:
 *   iterations: Number of times to run tests (default: 10)
 *   pattern: Test file pattern to run (default: all tests)
 *
 * Examples:
 *   npm run test:flaky
 *   npm run test:flaky 5
 *   npm run test:flaky 3 "api/**"
 *
 * Exit codes:
 ...
 */
function runTests(iteration: number, pattern = ''): JestResults {
 ...
 */
function extractTestResults(jestResults: JestResults): Map<string, TestResult> {
 ...
 */
function findFlakyTests(allResults: JestResults[]): FlakyTest[] {
 ...
 */
function printFlakyTestReport(flakyTests: FlakyTest[], iterations: number): void {
  print('\n═══════════════════════════════════════════', 'magenta');
  ...
  flakyTests.forEach((test, index) => {
    const reliability: ColorName = test.passRate >= 75 ? 'yellow' : 'red';
 ...
 */
function printRecommendations(flakyTests: FlakyTest[]): void {
 ...
 */
function saveFlakyTestReport(flakyTests: FlakyTest[], iterations: number): void {
  const reportDir = path.join(process.cwd(), 'test-reports');
  const reportPath = path.join(reportDir, 'flaky-tests.json');
...
async function main(): Promise<void> {
  ...
  const iterations = args[0] ? parseInt(args[0], 10) : 10;
  const pattern = args[1] || '';

  // Validate iterations
  if (isNaN(iterations) || iterations < 2 || iterations > 20) {
    print('❌ Error: Iterations must be between 2 and 20', 'red');
    process.exit(1);
  }
