...
interface GenerateImageTabProps {
  projectId: string;
}

/**
 * Generate Image Tab Component
 *
 ...
 */
export function GenerateImageTab({ projectId }: GenerateImageTabProps): React.ReactElement {
  ...
  const [safetyFilterLevel, setSafetyFilterLevel] = useState<'block_some' | 'block_few'>(
    'block_some'
  );
  ...
  const handleGenerateImage = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        setImageQueue((prev) =>
          prev.map((img) => (img.id === imageId ? { ...img, status: 'generating' } : img))
        );
        ...
        const imageUrls =
          json.assets
            ?.map((asset: { metadata?: { sourceUrl?: string } }) => asset.metadata?.sourceUrl)
            .filter(Boolean) || [];

        // Update queue item to completed
        setImageQueue((prev) =>
          prev.map((img) => (img.id === imageId ? { ...img, status: 'completed', imageUrls } : img))
        );
  ...
  const handleClearCompleted = useCallback(() => {
    setImageQueue((prev) =>
      prev.filter((img) => img.status !== 'completed' && img.status !== 'failed')
    );
  }, []);
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_600px] gap-6 h-full">
          ...
          <div className="overflow-y-auto">
            {/* Header */}
            <div className="mb-6">
              <h1 className="text-2xl font-bold text-neutral-900 mb-2">Generate Images with AI</h1>
              <p className="text-sm text-neutral-600">
                Create high-quality images using Google Vertex AI Imagen models. Customize every
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
                    ...
                    <p className="mt-2 text-xs text-neutral-500">
                      Describe the image you want to generate in detail. Include style, mood,
                  ...
                  <div className="rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
                    ...
                    >
                      <option value="imagen-3.0-generate-001">
                        Imagen 3.0 - Stable & High Quality
                      </option>
                      <option value="imagen-3.0-fast-generate-001">
                        Imagen 3.0 Fast - Faster Generation
                      </option>
                      <option value="imagegeneration@006">Imagen 4 - Latest Model (Preview)</option>
                    </select>
                    <p className="mt-2 text-xs text-neutral-500">
                      Choose between Imagen 3 (stable) or Imagen 4 (latest preview). Fast variants
                  ...
                  <div className="rounded-lg border border-neutral-200 bg-white p-6 shadow-sm space-y-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        value={aspectRatio}
                        onChange={(e) =>
                          setAspectRatio(e.target.value as '1:1' | '9:16' | '16:9' | '3:4' | '4:3')
                        }
                        disabled={generating}
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
                    ...
                    />
                    <p className="mt-2 text-xs text-neutral-500">
                      Describe what you DON&apos;t want in the image. The model will avoid these
                      elements.
                    </p>
                  </div>
                  ...
                  <div className="rounded-lg border border-neutral-200 bg-white p-6 shadow-sm space-y-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setSafetyFilterLevel(e.target.value as 'block_some' | 'block_few')
                        }
                        disabled={generating}
                        className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm text-neutral-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-colors disabled:cursor-not-allowed disabled:opacity-50"
                      >
                ...
                <div className="grid grid-cols-2 gap-4">
                  {imageQueue.map((image) => (
                    ...
                    >
                      ...
                      {/* Image Info */}
                      <div className="p-3">
                        <p className="text-xs text-neutral-700 line-clamp-2 mb-2">{image.prompt}</p>

                        {image.error && <p className="text-xs text-red-600 mb-2">{image.error}</p>}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {image.status === 'completed'
                              ? `${image.imageUrls?.length || 0} Generated`
                              : image.status === 'generating'
                                ? 'Generating'
                                : image.status === 'failed'
                                  ? 'Failed'
