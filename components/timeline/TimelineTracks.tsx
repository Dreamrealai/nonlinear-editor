...
import { trackRenderTime } from '@/lib/utils/performance';
...
export const TimelineTracks = React.memo<TimelineTracksProps>(function TimelineTracks({
  ...
  // Track render completion and send metrics
  useEffect((): void => {
    if (renderStartRef.current > 0) {
      const duration = performance.now() - renderStartRef.current;
      trackRenderTime('TimelineTracks', duration, {
  ...
  return (
    <>
      {Array.from({ length: numTracks }).map(
        (_, trackIndex): React.ReactElement => (
          <div
            key={trackIndex}
